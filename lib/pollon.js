...
class Pollon extends EventEmitter {
  ...
  _poll() {
    ...
    Pollon
      ...
      .then(() => function(done) { Pollon._handleBatchDataBound().then(r => done(null, r)) })
      .catch(err => {
        return Pollon.emit('error', new Error(`Generic error polling data: ${err.message}`))
      })
  }
  ...
  _handleBatchData(batch) {
    ...
    if (batch && batch.data && batch.data.length > 0) {
      const fnPromises = batch.data.map(singleData => Pollon._processSingleDataBound(singleData))

      let promise = fnPromises[0]()
      for (var i = 1; i < fnPromises.length; i++) {
          promise = promise.then(fnPromises[i]);
      }
